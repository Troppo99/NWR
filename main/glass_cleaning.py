            ...
            if i == 9:
                ...
                for idx, border in enumerate(scaled_borders_pts):
                    ...
                    if dist >= -radius:
                        ...
                        overlap = True
                if overlap:
                    keypoint_overlap = True  # Keypoints are overlapping with some border
...
def process_frames(frame_queue):
    global stop_flag, keypoint_overlap, keypoint_absence_timer_start
    ...
    while not stop_flag:
        if not frame_queue.empty():
            ...
            keypoint_overlap = False  # Reset overlap flag
            ...
            if any(state["is_green"] for state in border_states):
                if keypoint_overlap:
                    ...
                    if keypoint_absence_timer_start is None:
                        keypoint_absence_timer_start = current_time
                    elif (current_time - keypoint_absence_timer_start) >= 3:
                ...
                keypoint_absence_timer_start = None  # No green borders, no need to reset
